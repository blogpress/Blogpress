...
class Users {
	...
	function __construct(){
		...
		$dbServer = 'blogpress'; //Define database server host
		$dbUsername = 'blogpress'; //Define database username
		$dbPassword = 'blogpress'; //Define database password
		$dbName = 'blogpress'; //Define database name
	...
	function checkUser($oauth_provider,$oauth_uid,$username,$first_name,$last_name,$locale,$oauth_token,$oauth_secret,$profile_image_url){
		...
		if(mysqli_num_rows($prevQuery) > 0){
			...
			$insert = mysqli_query($this->connect,"INSERT INTO $this->tableName SET oauth_provider = '".$oauth_provider."', oauth_uid = '".$oauth_uid."', username = '".$username."', first_name = '".$first_name."', last_name = '".$last_name."', locale = '".$locale."', oauth_token = '".$oauth_token."', oauth_secret = '".$oauth_secret."', picture = '".$profile_image_url."', created = '".date("Y-m-d H:i:s")."', modified = '".date("Y-m-d H:i:s")."'") or die(mysqli_error($this->connect));
		}
